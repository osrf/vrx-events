...
def main():
    ...
    trees = [(scan[0], extract_trees(scan[1:])) for scan in laser_data]
    trees = [[dt] + (list(t.flatten())) for dt, t in trees if len(t) != 0]
    with open('LASER_TREE.txt', 'w') as f:
        for tree in trees:
            f.write("\t".join([str(t) for t in tree]))
            f.write('\n')
